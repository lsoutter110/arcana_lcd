...
void lcd_draw_sprite_scale(const Sprite sprite, const uint8_t x_pos, const uint16_t y_pos, const uint8_t x_scale, const uint8_t y_scale) {
	lcd_mem_write_begin(x_pos, y_pos, x_pos+(sprite.w*x_scale)-1, y_pos+(sprite.h*y_scale)-1);

	uint16_t *row_ptr = sprite.data;
	for(uint16_t y=y_pos; y<y_pos+sprite.h; y++) {
		for(uint8_t y_r=0; y_r<y_scale; y_r++) {
			uint16_t *data_ptr = row_ptr;
			for(uint8_t x=x_pos; x<x_pos+sprite.w; x++) {
				uint16_t col = *(data_ptr++);
